...
if postcode:
    # Clean up the postcode input
    cleaned_postcode = postcode.strip().replace(" ", "")
    
    try:
        lat, lon = get_geocoding_data(cleaned_postcode)
        if lat is not None and lon is not None:
            pm2_5 = get_air_pollution_data(lat, lon)
            alert, color = get_alert_level(pm2_5)

            # Display the results
            st.markdown(f"<h2 style='color: red;'>Current Burner Alert Guideline for: {postcode}</h2>", unsafe_allow_html=True)
            if color == "green":
                st.markdown(f"<div style='color:green; border: 2px solid green; border-radius: 5px; padding: 10px;'>Particle pollution on your street is well below guideline levels. Air quality is not currently unhealthy, although woodburner use may increase levels in your area.</div>", unsafe_allow_html=True)
            elif color == "yellow":
                st.markdown(f"<div style='color:orange; border: 2px solid orange; border-radius: 5px; padding: 10px;'>Particle pollution on your street is approaching guideline levels. Please consider not lighting your woodburner, particularly if you have an alternative source of heating.</div>", unsafe_allow_html=True)
            elif color == "red":
                st.markdown(f"<div style='color:red; border: 2px solid red; border-radius: 5px; padding: 10px;'>Particle pollution in this area is already above guideline levels. Avoid lighting your stove unless you do not have an alternative source of heating.</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='color:gray; border: 2px solid gray; border-radius: 5px; padding: 10px;'>Data not available for this location.</div>", unsafe_allow_html=True)

            # Display the PM2.5 value
            if pm2_5 is not None:
                st.markdown(f"<h3 style='color: red;'>PM2.5 Level: {pm2_5} µg/m³</h3>", unsafe_allow_html=True)
            else:
                st.markdown(f"<h3 style='color: red;'>PM2.5 Level: Data not available</h3>", unsafe_allow_html=True)
